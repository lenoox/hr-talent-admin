...
export class UserService {
  ...
  loginUser(data:UserRequest) {
    return this.http.post<UserResponse>(`${environment.apiUrl}/authentication/log-in`,data,httpOptions);
  }

  generateQr() {
     return this.http.post(`${environment.apiUrl}/2fa/generate`, {}, {
        responseType: 'blob',
      });
  }
  twoFactorAuthenticate(data:TwoFactorAuthenticateRequest) {
    return this.http.post<UserResponse>(`${environment.apiUrl}/2fa/authenticate`,data);
  }
  turnOn2fa(data:TurnOn2faRequest) {
    return this.http.post<UserResponse>(`${environment.apiUrl}/2fa/turn-on`,data);
  }
  refreshToken() {
