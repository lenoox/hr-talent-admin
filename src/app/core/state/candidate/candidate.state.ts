...
import {GetCandidate, GetCandidates, UpdateCandidate} from "./candidate.action";
import {Injectable} from "@angular/core";
import {Paginated} from "../paginated";
import {CandidateService} from "../../services/candidate.service";
import {candidateToList} from "../../../shared/mappers/candidates-mapper";
import {currentPageDefault, pageSizeDefault} from "../../constants/pagination.const";
...
export class CandidateState {
  ...
  @Action(GetCandidate)
  getCandidate({getState,setState}: StateContext<CandidatesStateModel>, { id }: CandidateResponse){
    return this.candidateService.fetchCandidate(id).pipe(tap((result)=>{
  ...
  getCandidates({getState,setState}: StateContext<CandidatesStateModel>,
    ...
    return this.candidateService.fetchCandidates(pageSize,currentPage).pipe(
      tap((result:Paginated<CandidateResponse>)=>{
        const state = getState();
        const candidatesList=result.items.map((results)=>candidateToList(results))
        setState({
          ...state,
          candidates: {
  ...
  updateCandidate({getState, setState}: StateContext<CandidatesStateModel>, {payload, id}: UpdateCandidate){
    return this.candidateService.updateCandidate(payload, id).pipe(
      mergeMap(()=>{
          return this.candidateService.fetchCandidates(pageSizeDefault,currentPageDefault)
        }
      ),
      tap(result=>{
        const state = getState();
        const candidatesList=result.items.map((candidates)=>candidateToList(candidates));
        setState({
          ...state,
          candidates: {
            items: candidatesList,
